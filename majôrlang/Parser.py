...
class Parser():
    ...
    def expression(self):
        ...
        while True:
            if self.match('MAIOR'):
                ...
                operator = '<'
            elif self.match('IQUAL'):
                operator = '=='
            elif self.match('MAIORIQUAL'):
                operator = '>='
            elif self.match('MENORIQUAL'):
                operator = '<='
            elif self.match('SOM'):
                operator = '+'
            elif self.match('SUB'):
                ...
                operator = '**'
            elif self.match('bitwise_AND'):
                operator = '&'
            elif self.match('bitwise_OR'):
                operator = '|'
            elif self.match('bitwise_XOR'):
                operator = '^'
            elif self.match('bitwise_NOT'):
                operator = '~'
            elif self.match('bitwise_Shift_Right'):
                operator = '>>'
            elif self.match('bitwise_Shift_Left'):
                operator = '<<'
            else:
                break
    ...
    def primary_expression(self):
        print(self.tokens[self.Index -1],self.tokens[self.Index],self.tokens[self.Index +1])
    ...
    def check_type(self, expected_type, value):
        if isinstance(value, Number):
            for i in ('INT8','INT16','INT32','INT64','BIN8','BIN16', 'BIN32','BIN64'):
